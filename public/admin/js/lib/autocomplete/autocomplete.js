...
(function($) {
    ...
    var TagSuggest = function(element, options) {
        ...
        var defaults = {
             ...
             */
            emptyText: function() {
                return cfg.editable ? 'ترکیب مورد نظر را وارد کنید' : 'Click here';
            },
...
$(document).ready(function() {
    var jsonData = [];
    var includes = myvariable.split(',');
    for (var i = 0; i < (includes.length); i += 2) jsonData.push({ id: includes[i + 1], name: includes[i] });
    var include = $('#ms1').tagSuggest({
        data: jsonData,
        sortOrder: 'name',
        maxDropHeight: 200,
        name: 'include'
